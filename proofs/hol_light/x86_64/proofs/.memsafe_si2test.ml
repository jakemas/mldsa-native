(* Setup-only: reload eta4 + all MEMSAFE infra, leave session ready. No test. *)
let dlog s = let oc = open_out_gen [Open_append;Open_creat] 0o644 "/tmp/memsafe_setup.log" in
             output_string oc (s ^ "\n"); close_out oc;;
(let oc = open_out "/tmp/memsafe_setup.log" in output_string oc "SETUP START\n"; close_out oc);;
...
dlog "SETUP DONE; running SI2 test";;
(let r =
   (try let _ = prove(clean_body_ms_tm,
          PREFIX_G_FULL_MS_TAC THEN SI1_FOLD_V2 THEN SI2_INTEGRATED_MS THEN
          (fun g -> failwith "STOP-after-SI2")) in "unexpected"
    with Failure m -> m | e -> Printexc.to_string e) in
 dlog ("SI2 TEST RESULT: "^r));;
dlog "DONE";;
