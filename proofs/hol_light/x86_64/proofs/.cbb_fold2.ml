let CBB_FOLD : tactic =
  SUBGOAL_THEN
   ...
   THENL
    [ALL_TAC;
     FIRST_X_ASSUM(fun th -> if is_forall(concl th) && can(find_term(fun u->u=`f0sub:int256`))(concl th) && can(find_term(fun u->match u with Const("word_sub",_)->true|_->false))(concl th) then MATCH_ACCEPT_TAC th else NO_TAC)];;
