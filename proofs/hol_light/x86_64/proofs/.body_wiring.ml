(* Wiring CLEAN_BODY into MLDSA_REJ_UNIFORM_ETA4_CORRECT.
   ...
   `if i+1<N then pc+56 else pc+318`). CONTINUE_TAC discharges the i+1<N branch via CLEAN_BODY +
   ...
   (same SIMD processing, but ending with the loop guard TAKEN to pc+318; store/RAX bounds hold
...
let CONTINUE_TAC : tactic =
  SUBGOAL_THEN `(if i + 1 < N then pc + 56 else pc + 318) = pc + 56` SUBST1_TAC THENL
