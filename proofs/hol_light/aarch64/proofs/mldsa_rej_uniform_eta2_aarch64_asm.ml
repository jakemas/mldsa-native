...
(* ========================================================================= *)
(* Secret-independent-timing (constant-time) proof for                        *)
(* rej_uniform_eta2_aarch64_asm  --  pq-code-package/mldsa-native issue #1160. *)
(* Adds the f_events clause (MLDSA_REJ_UNIFORM_ETA2_SUBROUTINE_SAFE) on top of *)
(* the functional-correctness + memory-safety theorems above.                 *)
(* ========================================================================= *)

(* ------------------------------------------------------------------------- *)
(* THE SECURITY PROPERTY (leakage-up-to-the-reject-mask, NOT obliviousness).  *)
(*                                                                            *)
(* rej_uniform_eta2 produces the SECRET eta coefficients (part of s1/s2), so  *)
(* its microarchitectural event trace must be independent of the secret       *)
(* *values*. It is however variable-time: the trace legitimately depends on   *)
(* WHICH candidate nibbles are accepted vs. rejected (the reject decision is   *)
(* public — a statistically-independent function of the public XOF stream,    *)
(* Round3 spec §5.5). So we prove leakage-up-to-the-reject-mask: the trace is  *)
(* a function of the public pointers PLUS the per-nibble accept/reject mask,   *)
(* and NOTHING else about the input (in particular, not the accepted values). *)
(*                                                                            *)
(* This is exactly the upstream TODO in rej_uniform_eta2_aarch64_asm.ml:      *)
(*   "the event-generating function should take the set of in-bound indices   *)
(*    as an argument, but not the entire input data. This corresponds to      *)
(*    declassifying WHICH coefficients are in bounds vs out of bounds, but     *)
(*    not their values."                                                      *)
(* ------------------------------------------------------------------------- *)

(* ------------------------------------------------------------------------- *)
(* WHY THE VANILLA mk_safety_spec DOES NOT WORK HERE.                         *)
(*                                                                            *)
(* gen_mk_safety_spec (s2n-bignum common/consttime.ml:135-148) derives        *)
(* f_events' public arguments as ONLY the base pointers + size vars of the    *)
(* mem-in/out/temp buffers, plus pc, the stack base pointer, and              *)
(* returnaddress. For eta2 that is [res; buf; buflen; table; pc;              *)
(* stackpointer; returnaddress] — the buffer CONTENTS are absent. The         *)
(* resulting spec would assert e2 = f_events <pointers only>, i.e. FULL       *)
(* data-obliviousness. That is FALSE for a rejection sampler: two inputs with *)
(* the same pointers but different reject patterns emit different traces      *)
(* (different table-lookup addresses table+mask*16, different store           *)
(* advances). No such data-dependent f_events precedent exists anywhere in    *)
(* s2n-bignum (grep for f_events taking a list/mask arg = 0 hits), so this    *)
(* spec is HAND-WRITTEN, adding the reject mask as an extra public argument.  *)
(* ------------------------------------------------------------------------- *)

(* ------------------------------------------------------------------------- *)
(* WHERE THE DATA-DEPENDENCE LIVES (from the .S, lines 54-91, loop8).         *)
(*                                                                            *)
(*   prologue (sub sp,#0x240) + Lrej_uniform_eta2_initial_zero (8 iters):     *)
(*       zero the 512-byte stack scratch. FIXED trip count. PUBLIC.           *)
(*   Lrej_uniform_eta2_loop8 (the sampler, DATA-DEPENDENT):                   *)
(*       ld1 {v0.8b},[x1],#8            read buf+8*i        (addr FIXED)      *)
(*       cmhi v4,v30,v16 ; and v31 ; uaddlv  -> w12 = 8-bit accept mask hi    *)
(*       cmhi v5 ...      ; uaddlv           -> w13 = 8-bit accept mask lo    *)
(*       ldr q24,[x3,x12,lsl#4]         read table+mask1*16 (addr = mask1)    *)
(*       ldr q25,[x3,x13,lsl#4]         read table+mask2*16 (addr = mask2)    *)
(*       cnt/uaddlv -> popcounts; tbl (compact); st1 [x7]; add x7,popcnt*2    *)
(*       st1 {v16.8h},[x7] ; st1 {v17.8h},[x7]  writes sp+2*ctr (addr = ctr)  *)
(*       add x9,ctr ; cmp x2,#8 ; b.hs loop8       trip count = f(mask,buflen)*)
(*   Lrej_uniform_eta2_final_copy (16 iters):                                 *)
(*       Barrett reduce scratch -> centered eta; store to res. FIXED. PUBLIC. *)
(*                                                                            *)
(* => The ONLY secret-dependent addresses/branches are: table+mask*16 reads,  *)
(*    sp+2*ctr stores, and the loop8 trip count. All three are functions of   *)
(*    the per-8-nibble accept masks (ctr = prefix popcount of the masks) and  *)
(*    the public buflen. Nothing depends on the accepted VALUES.              *)
(* ------------------------------------------------------------------------- *)
...
(* Reject mask distributes over APPEND (so a prefix mask composes cleanly;
   mirrors REJ_NIBBLES_ETA2_APPEND for the value-free projection). *)
let REJ_MASK_ETA2_APPEND = prove
 (`!l1 l2. REJ_MASK_ETA2(APPEND l1 l2) =
           APPEND (REJ_MASK_ETA2 l1) (REJ_MASK_ETA2 l2)`,
  REWRITE_TAC[REJ_MASK_ETA2; NIBBLES_OF_BYTES_APPEND; MAP_APPEND]);;
...
(* idx0 = FMOV(UADDLV(AND(CMHI(Q30=15-per-lane, nibbles),Q31=[1;2;4;..;128]))) *)
(* i.e. the WEIGHTED 8-bit pack of the per-nibble accept mask (NOT popcount;   *)
(* the eta2.ml:2744 "popcount-accumulator" comment is stale — the real bound   *)
(* proof at 2754-2774 uses SUM_8_BIT_BOUND_POLY with weights <=128, val<256).  *)
(* with the 8 per-nibble accept predicates (val nibble_k < 15) ABSTRACTED to   *)
(* booleans b0..b7. Lane layout (from the word_join tree): b0 = highest lane = *)
(* weight 128, ..., b7 = lowest lane = weight 1.                               *)
(*                                                                            *)
...
(* 4-byte group = REJ_MASK_ETA2 of that group, and 128*b0+64*b1+..+bitval b7 = *)
(* PACK_MASK8 (that group's mask) [PACK_MASK8 above]. So val(idx0) — hence the *)
(* table-read address table+16*val(idx0) — is a function of the PUBLIC reject  *)
(* the 2nd 4 bytes; the same lemma applies.)                                   *)
(*                                                                            *)
(* NOTE(#1160): the LHS below is machine-generated (round-trip parse verified  *)
(* pre-WORD_SUBWORD_AND uaddlv form; consider distributing WORD_SUBWORD_AND to *)
(* a per-lane form (as MEMSAFE does at eta2.ml:2578 for the popcount) before   *)
(* migrating to canonical rej_uniform_eta2_aarch64_asm.ml.                     *)
(* ------------------------------------------------------------------------- *)
...
(* weight 1). NIBBLES_OF_BYTES [byte0;byte1;byte2;byte3] lists nibbles low-first *)
(* per byte: [byte0-lo; byte0-hi; byte1-lo; ...; byte3-hi] = [b7;b6;b5;b4;b3;b2; *)
(* b1;b0]. Hence val(idx0) = PACK_MASK8 (REJ_MASK_ETA2 <that 4-byte group>) once *)
(* the abstract booleans are instantiated to (val nibble_k < 15) — the core     *)
(* _SAFE proof does that instantiation using the eta2 nibble machinery.         *)
...
(* WHOLE reject mask (REJ_MASK_ETA2 inlist) as f_ev_loop8's mask argument.  If *)
(* that argument stays the COMPOUND term (REJ_MASK_ETA2 inlist), then `frees`  *)
(* extracts the atom `inlist`, which is NOT syntactically the compound arg, so *)
(*   TEST (s124, live): unify `f_ev table mask i = [EventLoad(table+16*        *)
(*   PACK_MASK8(SUB_LIST(16*i,16) mask),16)]` SUCCEEDS with atomic `mask`,     *)
(*   but the same with the compound (REJ_MASK_ETA2 inlist) FAILS.              *)
(*                                                                            *)
(*   `mask = REJ_MASK_ETA2 inlist`  (atomic var).  Then the postcondition      *)
...
(* the atomic `mask` via REJ_MASK_ETA2_SUB_LIST below:                         *)
(*   idx0 = PACK_MASK8(REJ_MASK_ETA2(SUB_LIST(8*i,4) inlist))                  *)
(*        = PACK_MASK8(SUB_LIST(16*i,8) mask)                                  *)
(*   idx1 = PACK_MASK8(SUB_LIST(16*i+8,8) mask)                                *)
(*   curlen (store advance base) =                                            *)
(*        NUM_ACCEPTED(REJ_MASK_ETA2(SUB_LIST(0,8*i) inlist))                  *)
(*      = NUM_ACCEPTED(SUB_LIST(0,16*i) mask)                                  *)
(* The `inlist`-only parts (curlist = REJ_NIBBLES_ETA2 .., memory contents)    *)
...
(* values).  It equals MEMSAFE's WOP-defined N (rej_uniform_eta2_aarch64_asm  *)
(* .ml:2442): the least n at which EITHER the input is exhausted              *)
(* (val buflen < 8*(n+1)) OR the output is full (256 <= accepted-so-far).     *)
(* Each loop iteration consumes 8 bytes = 16 nibbles, and the accepted count  *)
(* after n iterations = NUM_ACCEPTED of the first 16n mask bits, because      *)
(* LENGTH(REJ_NIBBLES_ETA2(SUB_LIST(0,8n) inlist)) =                          *)
(* NUM_ACCEPTED(SUB_LIST(0,16n)(REJ_MASK_ETA2 inlist))   [s117 count bridge]. *)
(* So this is exactly MEMSAFE's N whenever mask = REJ_MASK_ETA2 inlist, and   *)
...
(* ------------------------------------------------------------------------- *)
(*                                                                            *)
(* This is the analytical crux of loop8's DUAL-EXIT closure.  loop8 exits     *)
(* when EITHER the input is exhausted (remaining < 8, i.e. val buflen <       *)
(* 8*(i+1)) OR the output is full (256 <= accepted so far).  In the           *)
(* prove PC = (if i+1 < TRIP then pc1 else pc2), which requires connecting    *)
(* the two HARDWARE branch conditions (X2<8 at the bottom b.hs 0xf8; X9>=256  *)
(* at the top b.hs 0x6c) to the arithmetic predicate i+1 < TRIP.  This lemma  *)
(* is exactly that connector, derived from the minimality of TRIP:            *)
...
(*     invariant conjuncts `curlen < 256` (eta2.ml:2504) and `8*(i+1) <=      *)
(*     buflen` (eta2.ml:2508), but stated over the PUBLIC atomic mask via the *)
(*     s117 count bridge (LENGTH(REJ_NIBBLES_ETA2(SUB_LIST(0,8i) inlist)) =   *)
(*     NUM_ACCEPTED(SUB_LIST(0,16i)(REJ_MASK_ETA2 inlist))) rather than the   *)
...
(* simp chain (eta2.ml:2564-2569), the live idx0 = read X12 collapses to      *)
...
(* machinery + REJ_MASK_ETA2_SUB_LIST + `REJ_MASK_ETA2 inlist = mask` give    *)
...
(* BEFORE `add x7`.  (b) stores need `nonoverlapping (word pc,372)            *)
(* (stackpointer,576)` in context (from the theorem's ALL clause) AND the     *)
(* 2nd store additionally needs val len0<=8 (UADDLV_BOUND_LEMMA) + the        *)
(* MEMSAFE WORD_ADD_SHL1 rewrite (eta2.ml:2971) on the word_shl'd X7 offset,  *)
(* else "could not prove updates will not modify the program code".           *)
(*                                                                            *)
(* `8 <= val(read X2 <after>)`:                                               *)
...
(* to `i+1<k` via MLDSA_ETA2_LOOP8_TRIP + the count bridge, keeping mask      *)
(* ------------------------------------------------------------------------- *)

(* ------------------------------------------------------------------------- *)
(* THE DRAFTED SAFETY + SECRET-INDEPENDENT-TIMING THEOREM.                    *)
(*                                                                            *)
(* Shape mirrors MLDSA_REJ_UNIFORM_ETA2_SUBROUTINE_MEMSAFE                    *)
(* (rej_uniform_eta2_aarch64_asm.ml:3166) but:                               *)
(*   - wraps it in `exists f_events. forall ...` (like                        *)
(*   - adds the conjunct  e2 = f_events res buf buflen table pc stackpointer  *)
(*         returnaddress (REJ_MASK_ETA2 inlist)  inside `exists e2`,          *)
(*   - uses the trivial frame (\s s'. true) (keep_maychanges:false idiom).    *)
(*                                                                            *)
(* KEY: `inlist` (secret) enters f_events ONLY through (REJ_MASK_ETA2 inlist).*)
(* The accepted values (REJ_NIBBLES_ETA2 inlist / the s1/s2 coefficients) are *)
(* ABSENT from every f_events argument. That is the secret-independence claim.*)
(*                                                                            *)
(* consts; the real-const form below is a copy-paste with the actual names).  *)
(* ------------------------------------------------------------------------- *)
...
(* i+1<8, given i<8.  IVAL_WORD_SMALL is the standard `ival(word m)=&m` for   *)
(* m below the signed bound (2^63), used to evaluate both compared operands.  *)
(* ------------------------------------------------------------------------- *)
...
(* The signed b.lt back-edge condition for the FINAL-COPY (fcopy) counter: given *)
(* i<16 (loop bound), the raw stepper flag condition on the post-increment       *)
(* counter 16*(i+1) is equivalent to the loop-continuation predicate i+1<16.     *)
(* This is FCOPY's analogue of ZINIT_BRANCH (32*(i+1) vs 8): 16*(i+1) vs 256,     *)
(* -240 = word 18446744073709551376 (= 16 - 256 in two's complement).            *)
...
(* already-landed MLDSA_ETA2_LOOP8_TRIP_MINIMAL) needed to discharge loop8's  *)
(*                                                                            *)
(*   BOTTOM branch (0xf4 cmp x2,#8 ; 0xf8 b.cs 0x68), x2 = buflen-8*(i+1):     *)
(*     live form:  read PC = if 8 <= val(word_sub buflen (word(8*(i+1))))      *)
(*                          then word(pc+104)[=0x68] else word(pc+252)[=0xfc]  *)
...
(* =========================================================================== *)
(*                                                                             *)
(* was RUN LIVE (light env) and is GREEN end-to-end.  Confirmed:               *)
(*                                                                             *)
(*      into the loop8 slot (in place of the f_ev_loop8 meta-var); the 6 other *)
(*      oblivious segments keep their meta-vars.                               *)
(*      postcond subgoals (mk_new_inv, consttime.ml:792 — does NOT require a    *)
(*      (0x68/0x6c) — produces read PC / read events in EXACTLY the shape of    *)
(*      the baked e_loop (verified: the two EventJump targets are public        *)
(*      if-terms that match by REFL; no need to resolve the top-branch if in    *)
(*        (a) PC:  (if 256<=val(word(NUM_ACCEPTED(SUB_LIST(0,16*(i+1)) mask)))  *)
(*                  then word(pc+252) else word(pc+112))                       *)
(*                 = word(if i+1<TRIP then pc+112 else pc+252)                 *)
(*               + MLDSA_ETA2_LOOP8_TRIP_MINIMAL (part 2 at j=i+1 for the       *)
(*               continue arm; part 1 + the input-not-exhaust casehyp for the   *)
(*               trivial (loop8 events are all EventJumps, which                *)
(*               memaccess_inbounds ignores — safety.ml:68).                    *)
(*                                                                             *)
(* => Option A is viable; drive the FULL loop8 body (the ~11-min SIMD compute  *)
(*    0x70..0xf4, per the s125 recipe) in a DETACHED BATCH, entering the tail   *)
(*    the casehyp itself to `true`, losing it for the i+1=TRIP close) — keep a  *)
(*    assumptions).                                                            *)
(*                                                                             *)
(* 5 compute events buf-load/2 table-loads/2 stores, mask-bridged per s125):    *)
(*   \i. if 8 <= val(word_sub buflen (word(8*(i+1))):int64)                     *)
(*       then [EventJump(word(pc+108),                                          *)
(*               (if 256 <= val(word(NUM_ACCEPTED(SUB_LIST(0,16*(i+1)) mask)))   *)
(*                then word(pc+252) else word(pc+112)));                        *)
(*             EventJump(word(pc+248), word(pc+104))]     (* CaseA: 2 jumps *)  *)
(*       else [EventJump(word(pc+248), word(pc+252))]      (* CaseB: 1 jump  *)  *)
(* The loop8 UP2 invariant must add `NUM_ACCEPTED(SUB_LIST(0,16*i) mask) < 256` *)
(* (mirrors MEMSAFE niblen<256) so LOOP8_TOP_VAL_BRIDGE's <2^64 hyp discharges. *)
(* =========================================================================== *)

(* =========================================================================== *)
(*                                                                             *)
(* (Option A), now a real `prove` (NO SIMD, ~3s).  It is the reusable second   *)
(* X2 = buflen - 8*(i+1), X9 = curlen' = NUM_ACCEPTED(SUB_LIST(0,16*(i+1))mask),*)
(* the invariant registers threaded, events = e), it steps the two dual-exit   *)
(* branches (0xf4 cmp x2,#8; 0xf8 b.cs 0x68 = bottom/input-exhaust ; then in    *)
(* the input-not-exhausted arm 0x68 cmp x9,x4; 0x6c b.cs 0xfc = top/output-full)*)
(* and lands at word(pc + if i+1<TRIP then 0x70 else 0xfc) with the exact       *)
(* dual-exit EventJump trace (the CaseA/CaseB if-term validated s128).          *)
(*                                                                             *)
(* bottom hw cond `8 <= val(word_sub buflen (word(8*(i+1))))`; CaseA resolves    *)
(* re-ASSUME), steps the top branch (3--4), and closes PC via the equivalence    *)
(* 256<=val(word curlen') <=> ~(i+1<TRIP) [LOOP8_TOP_VAL_BRIDGE + LOOP8_OUTPUT_  *)
(* derives ~(i+1<TRIP) via LOOP8_BOT_VAL_BRIDGE + MINIMAL part 2 at i+1.         *)
(*                                                                             *)
(* KEY DESIGN CORRECTION (supersedes s128 Open Issue #2 above): the loop8 UP2   *)
(* invariant must NOT carry `NUM_ACCEPTED(SUB_LIST(0,16*i) mask) < 256`.         *)
(* (i+1)` to hold EVEN AT THE EXIT iteration (i+1 = TRIP).  At an output-full    *)
(* exit curlen(i+1) >= 256, so a <256 invariant conjunct would be FALSE there    *)
(* and the body would be unprovable.  Instead the tail lemma takes the loose     *)
(* `NUM_ACCEPTED(SUB_LIST(0,16*(i+1)) mask) < 2 EXP 64` as a HYPOTHESIS; step 2  *)
(* discharges it inside the SIMD body from `curlen < 256` (derived from i<k via  *)
(* MINIMAL part 2) + one iteration adds <= 16 accepts.  The `8*(i+1) <= val      *)
(* buflen` fact the bottom bridge needs is derived internally from i<TRIP.       *)
(*                                                                             *)
(* MUST be annotated `(buflen:int64)` — an un-annotated `val buflen < ...` in    *)
(* LOOP8_BOT_VAL_BRIDGE (int64) did not close it, and the stray open subgoal     *)
(*                                                                             *)
(* to this lemma's precond, apply it].  e_loop i then composes as APPEND         *)
(* =========================================================================== *)
...
(* ========================================================================= *)
(* REAL compute-lemma goal (closes the s125 "integration asserted not          *)
(* demonstrated" gap for the compute half, now on a concrete `ensures arm`     *)
(* whose postcond IS MLDSA_ETA2_LOOP8_TAIL_CLOSE's precond).                   *)
(*                                                                            *)
(* (no per-step SIMD simplification) + the MEMSAFE batch simp chains is FAST:  *)
(* Total interactive stepping to the 1st store ~30s (NOT 11 min).  Only        *)
(* whole compute prefix can be driven interactively OR in one modest batch.    *)
(*                                                                            *)
(* THE TARGET LEMMA (validated to type-check; postcond = TAIL_CLOSE precond    *)
(* + 5 explicit mask-form events; composes with MLDSA_ETA2_LOOP8_TAIL_CLOSE    *)
(*                                                                            *)
(*   MLDSA_ETA2_LOOP8_COMPUTE:                                                 *)
(*   !res buf buflen table inlist mask pc i e stackpointer.                    *)
(*     mask = REJ_MASK_ETA2 inlist /\ LENGTH inlist = val buflen /\            *)
(*     8*(i+1) <= val buflen /\ NUM_ACCEPTED(SUB_LIST(0,16*i) mask) < 256 /\   *)
(*     nonoverlapping (word pc,372) (word_sub stackpointer (word 576),576)     *)
(*         + read events = APPEND [st2; st1; ldtab1; ldtab0; ldbuf] e)         *)
(*         (\s s'. T)                                                          *)
(*   where the 5 events (newest-first) are, with                              *)
(*     curlen = NUM_ACCEPTED(SUB_LIST(0,16*i) mask):                          *)
(*     EventStore(sp-576 + 2*(curlen + NUM_ACCEPTED(SUB_LIST(16*i,8) mask)),16)*)
(*     EventStore(sp-576 + 2*curlen, 16)                                       *)
(*     EventLoad (table + 16*PACK_MASK8(SUB_LIST(16*i+8,8) mask), 16)          *)
(*     EventLoad (table + 16*PACK_MASK8(SUB_LIST(16*i,8) mask), 16)            *)
(*     EventLoad (buf + 8*i, 8)                                                *)
(*                                                                            *)
(*     EventStore(word_add(word_sub stackpointer(word 576))(word(2*curlen)),16)*)
(*                                                                            *)
(* REMAINING (all "known recipe", to finish MLDSA_ETA2_LOOP8_COMPUTE):         *)
(*  (1) bounds val(len0)<=8 / val(len1)<=8: after the 2nd simp the goal is     *)
(*      match (len0's collapsed form differs from the raw cnt/uaddlv shape of  *)
(*      `val(read X12)=LENGTH lis0` (eta2.ml:2529-2536,2869) idiom.            *)
(*  (3) MASK BRIDGES (the crux; s125 recipe, all lemmas already landed):       *)
(*      loaded_d = num_of_wordlist(SUB_LIST(8*i,8) inlist) via                 *)
(*        SUB_LIST_8_BYTES_FROM_INT64 (eta2.ml, needs 8*(i+1)<=LENGTH inlist); *)
(*        b_k to nibble preds (VAL_WORD_ZX_BYTE16/BYTE_AND_15_MOD/BYTE_USHR4_  *)
(*      len0/len1: LENGTH lis0 = NUM_ACCEPTED(REJ_MASK_ETA2(4-byte grp)) via   *)
(*        LENGTH_REJ_NIBBLES_ETA2_EQ_MASK + NUM_ACCEPTED + REJ_MASK_ETA2_SUB_  *)
(*  (4) POSTCOND close: X9/X7 final via NUM_ACCEPTED_SUB_LIST_STEP (below) +   *)
(*      WORD_RULE; events via (3); memory (table/buf) preserved (only stack    *)
(*      written).  Keep `mask` FOLDED throughout (atomic-mask discipline).     *)
(*                                                                            *)
(*     CLOSE ] ; the per-iter e_loop = APPEND <TAIL_CLOSE branch-jumps>        *)
(*   <the 5 compute events>; APPEND_ASSOC bookkeeping between the two halves.  *)
(* ========================================================================= *)

(* Count-bridge for the loop8 per-iteration accepted count (final X9 value and *)
(* the 2nd stack-store address): the accepted-count after i+1 groups = the     *)
(* count after i groups + the two 4-byte-half counts of group i.  Pure list    *)
...
(* Each 4-byte loop8 half-group accepts at most 8 nibbles (LENGTH bound). Used to *)
(* bound the per-iteration store advance so the 2nd stack store's address stays  *)
...
(* ========================================================================= *)
(*                                                                            *)
(* content): the loop8 SIMD reads 8 bytes into v0 then computes over them.     *)
(* (i.e. after the leading `sub x2` = step 1, before step 2), so the load      *)
(* folds to `read Q0 s2 = word_zx loaded_d` and read Q16 s9 (nibbles0) becomes *)
(* a PURE function of loaded_d.  If loaded_d is abbreviated AFTER the ld1       *)
(* (s130's earlier drive), idx0's accept bits reference `read Q0 s2` (an opaque *)
(* register atom, NOT loaded_d) and WORD_BLAST cannot connect the nibbles to   *)
(* loaded_d — the whole bridge stalls.  This was the true blocker.             *)
(*                                                                            *)
(*  2. at s9 (BEFORE reabbrev), establish 16 nibble facts by WORD_BLAST         *)
(*     `word_subword (read Q16 s9) (16k,16):int16 =                            *)
(*        word_zx(word_and/ushr (word_subword loaded_d (bp,8):byte) (word 15/4))` *)
(*     via FIRST_ASSUM(SUBST1 the `read Q16 s9 = <uxtl>` fact) THEN WORD_BLAST;  *)
(*     (bp = (k div 2)*8 for Q16 bytes 0-3, +32 for Q17 bytes 4-7; even k=and,  *)
(*     (These 16 facts survive because they mention only loaded_d + nibbles0.)  *)
(*  4. mask-slice fact `SUB_LIST(16*i,8) mask = [8 loaded_d-nibble bools]`:      *)
(*     - SUB_LIST(8*i,8) inlist = [8 subwords loaded_d] via                      *)
(*     - split with SUB_LIST_4_FROM_8; connect SUB_LIST(16*i,8) mask =           *)
(*       [REJ_MASK_ETA2;NIBBLES_OF_BYTES_4;MAP;VAL_WORD_NIBBLE_LT]).             *)
(*  ALL FIVE STEPS RAN CLEAN LIVE (s131).  ⚠ Do NOT bulk-discard assumptions by  *)
(*  size (`>3000 chars`) — that also nukes the 6.4KB idx0/idx1 raw forms; discard*)
(*  only the uxtl bigforms by matching the constant "1085102592571150095".       *)
(*                                                                            *)
(* REMAINING to finish MLDSA_ETA2_LOOP8_COMPUTE (NOT yet a landed prove):        *)
(*   — analogous but via the CNT/popcount form: POPCOUNT_AND_POWERS then         *)
(*   UADDLV_COUNT_LEMMA-style sum = NUM_ACCEPTED [8 bools] (mask slice);         *)
(*   val len<=8 is then free (NUM_ACCEPTED <= LENGTH <= 8); 2 stores (VERBOSE +  *)
(*   WORD_ADD_SHL1 + nonoverlapping (2nd-store-addr,16)(word pc,344) via         *)
(*   via the 4 bridges).  Then compose with MLDSA_ETA2_LOOP8_TAIL_CLOSE.         *)
(* ========================================================================= *)
...
(* accept predicates (low then high nibble of each byte, matching the SIMD lane *)
(* order after the AND/USHR/ZIP/UXTL chain).                                    *)
...
(* ========================================================================= *)
(* all 4 bridges (idx0/idx1/len0/len1) proven on the real COMPUTE goal and all   *)
(* 5 events materialised in ATOMIC-mask/PACK_MASK8 form.  Only the postcond      *)
(* clean-script pass from a landed prove.  Option A is CONVERGING, not a dead-   *)
(* end.  Full validated recipe below; assemble it as ONE clean prove (avoid the  *)
(* interactive fold back-and-forth that tangled s132's live state).              *)
(*                                                                            *)
(* ***THE ROOT-CAUSE FIX for the len0/len1 bridge (what s131 missed):*** the     *)
(* PER-BIT BITBLAST rules `bit k (word_subword (word_neg (word (bitval b):16     *)
(* word)) (0,8):8 word) <=> b` (eta2.ml:1756-1759 / 2798-2801).  s131's one-shot *)
(* (POPCOUNT_AND_POWERS + UADDLV_COUNT_LEMMA) omitted them and died on the        *)
(*     `bit k (word_subword (word_neg (word (bitval b):16 word)) (0,8):8 word)    *)
(*      <=> b`)) (0--7)) THEN                                                     *)
(*   FIRST_ASSUM(SUBST1 that len0-LENGTH fact) THEN                              *)
(*     th))]) THEN                                                               *)
(*     REJ_MASK_ETA2_SUB_LIST] THEN                                              *)
(* (len1 identical: nibbles1b, bytes (32,8)..(56,8), CONJUNCT2, 2*(8*i+4)=16*i+8).*)
(*                                                                            *)
(*  EXEC=MLDSA_REJ_UNIFORM_ETA2_EXEC.  Goal = the COMPUTE statement below.        *)
(*  5. PROVE_HW_ALL: 16 nibble facts word_subword nibbles0/1b (16k,16) =          *)
(*     word_zx(word_and/ushr (word_subword loaded_d (bp,8)) ...).  Use MEMSAFE's  *)
(*     prove_hw BUT with the check restricted to a NON-`read` LHS (the word_join  *)
(*  6. ***IDX BRIDGES HERE — BEFORE SIMP2*** (order fix; SIMP2's WORD_SUBWORD_AND *)
(*     DISTRIBUTES the `word_and (word 664..)` weight const into per-lane         *)
(*     `word_and (word 2^k)` so UADDLV_PACK_LEMMA no longer matches).  Establish  *)
(*           [ARITH`2*(8*i)=16*i`;ARITH`2*4=8`]; ALL] THEN                        *)
(*     (idx1: sub4hi, 2*(8*i+4)=16*i+8.)  These facts SURVIVE SIMP2.              *)
(*     [If you keep idx bridges AFTER SIMP2 instead, UADDLV_PACK_LEMMA fails; you *)
(*      must abstract the distributed form to a fresh 256-case BOOL_CASES lemma   *)
(*      `IDX_DISTRIB_PACK` (s132 proved one live) — doing it BEFORE SIMP2 avoids  *)
(*      that entirely.]                                                           *)
(*     (18--25); SIMP2 (eta2.ml:2578-2584); THEN the len0/len1 bridge + bounds +  *)
(*     len1` (NOT NUM_ACCEPTED) in the store-address arithmetic (see step 9).     *)
(*     base MUST be a plain var, else the store's nonoverlapping automation can't *)
(*     match `word_add (word_sub sp 576) X`).                                     *)
(*       (word_add sp0 (word(2*(curlen+val len0))),16)(word pc,344) [NONOVERLAP-  *)
(*       X9=word_add(word curlen)(word_add len0 len1), events all in PACK/curlen/ *)
(*     to split the postcond's NUM_ACCEPTED(SUB_LIST(0,16*(i+1)) mask); then fold *)
(*     X7 via ARITH assoc + WORD_RULE, X9 via `word_add(word c)(word_add a b) =   *)
(*     word(c+val a+val b)` (prove: len_k=word(val len_k) [WORD_VAL] then WORD_   *)
(*     ***Preserve the val-len0=NUM_ACCEPTED facts to the very end — do NOT       *)
(*     RULE_ASSUM(GSYM lenfact) mid-drive (it rewrites the lenfact to a trivial   *)
(*     `val len0 = val len0` and you lose the link).***                          *)
(*                                                                            *)
(*   MLDSA_ETA2_LOOP8_COMPUTE statement (validated to type-check + drive to       *)
(*     !res buf buflen table (inlist:byte list) mask pc i e stackpointer tabval.  *)
(*       mask = REJ_MASK_ETA2 inlist /\ LENGTH inlist = val buflen /\             *)
(*       8*(i+1) <= val buflen /\ NUM_ACCEPTED(SUB_LIST(0,16*i) mask) < 256 /\    *)
(*       nonoverlapping (word pc,372)(word_sub stackpointer (word 576),576)       *)
(*       ==> ensures arm (PC=pc+112, loop8 inv @ i, buf=num_of_wordlist inlist,   *)
(*           table=tabval) (PC=pc+244, inv @ i+1 + read events = APPEND           *)
(*           [Store(sp0+2*(NA(0,16i)+NA(16i,8)));Store(sp0+2*NA(0,16i));          *)
(*            Load(table+16*PACK_MASK8(SUB_LIST(16i+8,8) mask));                  *)
(*            Load(table+16*PACK_MASK8(SUB_LIST(16i,8) mask));Load(buf+8i)] e)    *)
(*           (\s s'. T)  [sp0 = word_sub stackpointer (word 576)].                *)
(*   <TAIL_CLOSE precond @ i> THEN [apply COMPUTE (tabval:=num_of_wordlist table, *)
(*   e_loop i = APPEND <TAIL_CLOSE 2 branch-jumps> <the 5 compute events>.        *)
(* ========================================================================= *)

(* ========================================================================= *)
(*                                                                            *)
(* leakage-up-to-the-reject-mask security content):                            *)
(* so every one of the 5 emitted events (buf load, 2 table loads, 2 stores) is  *)
(* an address that is a function of the PUBLIC (buf,table,sp,i,mask) only —      *)
(* never the accepted values.  Postcond = MLDSA_ETA2_LOOP8_TAIL_CLOSE's precond *)
(* at i+1 + those 5 events, so the loop8 body leaf composes as                  *)
(*                                                                            *)
(* Corrections to the s132 recipe found while assembling this as one prove:     *)
(*  - len bridge: the popcount reduces to LOADED_D form (`word_zx(word_and      *)
(*    (word_subword loaded_d ..)(word 15))`), NOT `word_subword nibbles0 ..`     *)
(*    form, so you MUST fold it back to nibbles0 form (GSYM of the 16 nibble     *)
(*    forms).  Folding the LENGTH form away with SUBST1 (keeping only the        *)
(*    NUM_ACCEPTED form) makes the 2nd stack store fail its program-code         *)
(*    non-modification check.                                                    *)
(*  - the idx bridges must run BEFORE SIMP2 (SIMP2 distributes the Q31 weight    *)
(*    const so UADDLV_PACK_LEMMA stops matching).                                *)
(*  - the nonoverlapping subgoal for the 2nd store needs explicit :int64         *)
(*    annotations, val len0<=8 (NUM_ACCEPTED_LE_8) and curlen<256 in context.    *)
(* ========================================================================= *)
...
(* the 16 per-nibble facts word_subword nibbles0/1b (16k,16) = word_zx(word_and/ushr..) *)
...
(* ========================================================================= *)
(* MLDSA_ETA2_LOOP8_COMPUTE o MLDSA_ETA2_LOOP8_TAIL_CLOSE.                     *)
(*                                                                            *)
(* FAIL-FAST COMPOSITION VERIFIED (the s133 reviewer's one open question):    *)
(* MLDSA_ETA2_LOOP8_TAIL_CLOSE's precond under                                *)
(*   [bufval := num_of_wordlist inlist, e := APPEND <5 compute events> e].    *)
(*  COMPUTE's postcond WITHOUT its two leading aligned/PC conjuncts.)         *)
(*                                                                            *)
(* The two side hyps are discharged from `i < TRIP` via TRIP_MINIMAL part 2   *)
(* (COMPUTE's `8*(i+1)<=buflen` and `NA(0,16i)<256`) and via                  *)
(* NUM_ACCEPTED_SUB_LIST_STEP + NUM_ACCEPTED_LE_8 (TAIL_CLOSE's `NA(0,16(i+1)) *)
(*  < 2 EXP 64`, bounded by 255+8+8), keeping `mask` ATOMIC throughout (never *)
(*                                                                            *)
(* Per-iteration public event trace                                          *)
(*   e_loop i = APPEND <2 dual-exit branch jumps> <the 5 compute events>.     *)
(* The goal is built PROGRAMMATICALLY from COMPUTE + TAIL_CLOSE (mirrors       *)
(* UADDLV_PACK_LEMMA's machine-generated construction in this file) so the    *)
(* two halves are guaranteed to line up by construction.                      *)
(* ========================================================================= *)
let MLDSA_ETA2_LOOP8_BODY =
  let strip_PQR th =
  ...
  let (tP,tQ,tR) = strip_PQR MLDSA_ETA2_LOOP8_TAIL_CLOSE in
  ...
  GEN_ALL(prove(mk_imp(bl_hyps, bl_ensures),
    ...
    CONJ_TAC THENL
     [ MATCH_MP_TAC MLDSA_ETA2_LOOP8_COMPUTE THEN
       ...
       MATCH_MP_TAC MLDSA_ETA2_LOOP8_TAIL_CLOSE THEN
       CONJ_TAC THENL
        [ FIRST_ASSUM ACCEPT_TAC;
...
(* ========================================================================= *)
(* deep, with the exact next blocker pinned.                                  *)
(*                                                                            *)
(* TASK 1 (COMMITTED, gated): COMPUTE / TAIL_CLOSE / BODY above now carry     *)
(*   `read SP s = word_sub stackpointer (word 576)` + `read X30 s =           *)
(*   returnaddress` in BOTH pre and post (+ a `returnaddress` quantifier).    *)
(*   handling (COMPUTE re-proves ~54s; TAIL_CLOSE ~3s; BODY re-derives 0.2s   *)
(*   from its programmatic construction, unchanged). This makes the loop8     *)
(*                                                                            *)
(* split; the remaining crux precisely pinned.  Reproduction recipe:          *)
(*  (a) Modified CONCRETIZE lambda: splice BODY's per-iteration event delta   *)
(*      into the `f_ev_loop8` slot.  delta i = APPEND <2 dual-exit jumps>     *)
(*      <5 compute events>, extracted from BODY's postcond `read events`      *)
(*      conjunct (rhs = APPEND A (APPEND B e); delta = APPEND A B), then       *)
(*      the outer lambda + subst in the OPEN body (NOT subst under the binder *)
(*      — capture-avoiding subst won't match the bound-var'd f_ev_loop8_abs), *)
(*      then re-abstract.  Front (CONCRETIZE modified_lam THEN META_EXISTS    *)
(*      (APPEND epil (APPEND loop prol)) tail) tail2 /\ Q e2) <=> ...          *)
(*      APPEND (APPEND epil (APPEND loop prol)) (APPEND tail tail2) ...`       *)
(*      rebracket (bignum_copy_row_from_table.ml:771) — but apply it with     *)
(*      associates the 4-deep trace into `APPEND EPIL (APPEND L8 ACC)`, so    *)
(*      L8 with count = MLDSA_ETA2_LOOP8_TRIP buflen mask.                     *)
(*      aligned/PC/events, tabval := num_of_wordlist eta_table, as `\i s.`)   *)
(*      `\s. aligned /\ PC=word(pc+112) /\ (GROUPED loopinv conjunction) /\   *)
(*      e_loop) acc)` — this does NOT flat-match MLDSA_ETA2_LOOP8_BODY's      *)
(*      precond `\s. aligned /\ PC /\ SP /\ X30 /\ ...flat... /\ read events  *)
(*      directly (validated live — no match).  The mismatch is (i) loopinv    *)
(*      grouped vs flat and (ii) events-exists vs a direct `read events s=e`. *)
(*      mask bridges BODY was built to avoid.  So the reuse path needs an     *)
(*      ENSURES precond/postcond MONOTONICITY bridge: instantiate BODY's `e`  *)
(*      and show P_goal ==> P_body (grouped=>flat, events-exists picks e) and *)
(* ========================================================================= *)
...
(* pth) — which does NOT flat-match MLDSA_ETA2_LOOP8_BODY's precond.           *)
(*                                                                            *)
...
(*      Post-weaken: PC bridge (word(pc+if..)=word(if..then pc+112..) via      *)
...
(*                                                                            *)
(* MLDSA_ETA2_LOOP8_BODY_ADAPTED is stated with e_front/acc as FREE vars       *)
(* its 3 ambient hyps (mask=REJ_MASK_ETA2 inlist, LENGTH inlist=val buflen,    *)
(* ALL nonoverlapping) discharge from the main-proof context.  conj2 is built  *)
(* (no SIMD), guaranteeing the statement is exactly the shape the front emits. *)
(* ========================================================================= *)
...
(* Restore the recursive `STRIP_EXISTS_ASSUM_TAC` from s2n-bignum common/misc.ml,        *)
(* which the constant-time proof below was developed against.  The MEMSAFE section       *)
(* (near line 2363) locally redefines STRIP_EXISTS_ASSUM_TAC to a *shallow* variant that  *)
(* only splits the outermost conjunction of the chosen existential.  The event-loop       *)
(* leaves below feed `~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC)` an invariant of the    *)
(* form `?e2. read events s = ... /\ e2 = ... /\ memaccess_inbounds e2 ...`; the shallow   *)
(* variant leaves `e2 = ... /\ memaccess_inbounds e2 ...` conjoined, so                    *)
(* DISCHARGE_MEMACCESS_INBOUNDS_USING_ASM_TAC (which filters assumptions by head symbol)   *)
(* cannot see the `memaccess_inbounds e2` fact and the discharge fails.  The recursive     *)
(* STRIP_ASSUME_TAC form splits it into a standalone assumption.                           *)
let STRIP_EXISTS_ASSUM_TAC =
...
let MLDSA_REJ_UNIFORM_ETA2_SUBROUTINE_SAFE =
  ...
  (* MINIMAL fcopy invariant (postcond doesn't constrain regs; omit X9/v-consts). *)
  let fcopy_inv =
